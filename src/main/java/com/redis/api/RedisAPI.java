...
//	      System.out.println("Server is running: "+jedis.ping());
		   ...
		   System.out.println(RedisClient.getRedisClient().getJedis());  //针对单个服务器，不是集群的形式
	}
	private Jedis redis ;
...
//	    redis.auth("123456");
	...
	public void HashApi(){
		...
		public void SortedSetApi(){
			...
			print("名为zsets添加数据value4："+redis.zadd("zsets",3, "value4"));
		...
		public void RedisClusterDemo(){
			...
			long seccond = t2/1000;
			long shi = seccond/3600;
			long fen = ((seccond - shi*3600) /60);
			long miao = seccond - shi*3600 - fen*60;
			String str = shi +" 小时      "+fen+" 分 "+miao+"   秒";
			System.out.println(str);
			System.out.println("第100 的值是 ： "+jc.hget("vv","s100"));
		}

		@Test
		public void sharedPoolTest(){
			ShardedJedis shardedJedis = RedisClient.getRedisClient().getShardedJedis();
			//String
			shardedJedis.set("ss","kkk");
			shardedJedis.get("ss");

			//hash
			shardedJedis.hset("student","tom","12");
			shardedJedis.hget("student","tom");
